...
class UserBaseSchema(BaseModel):
    email: str = Field(..., max_length=170, description="use email for validate user")
    full_name: str = Field(..., max_length=200, description="set fullname for user")
    national_id: str = Field(..., max_length=10, min_length=0)
    password: str = Field(max_length=70, min_length=8)
    password_confirm: str = Field(
        max_length=70, min_length=8, description="password confirmation"
    )
